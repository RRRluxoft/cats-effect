...
object RacingIOs extends IOApp.Simple {
  ...
  def testRace() = {
    ...
    val first: IO[Either[Int, String]] = // IO.race(meaningOfLife, favLang) //v1
      unrace(meaningOfLife, favLang)     // v2
    /*
  ...
  def timeout[A](io: IO[A], duration: FiniteDuration): IO[A] = {
    ...
    val result: IO[Either[Unit, A]] = IO.race(timeout, io)

    result.flatMap {
      case Left(_)   => IO.raiseError(new RuntimeException("Computation timed out."))
      case Right(ev) => IO(ev)
    }.onCancel(IO(s"Cancelled").void)
  }

  val importantTask = IO.sleep(2.seconds) >> IO(42).debug
  val testTimeout = timeout(importantTask, 5.seconds)
  ...
  def unrace[A, B](ioa: IO[A], iob: IO[B]): IO[Either[A, B]] = IO.racePair(ioa, iob).flatMap {
    ...
    case Right((fibA, _)) => fibA.join.flatMap {
        case Succeeded(effA) => effA.map(a => Left(a))
        case Errored(e)      => IO.raiseError(e)
        case Canceled()      => IO.raiseError(new RuntimeException("Loser canceled."))
      }
  }
  ...
  def simpleRace[A, B](ioa: IO[A], iob: IO[B]): IO[Either[A, B]] = IO.racePair(ioa, iob).flatMap {
    case Left((outA, fibB))  => outA match {
        case Succeeded(effectA) => fibB.cancel >> effectA.map(e => Left(e))
